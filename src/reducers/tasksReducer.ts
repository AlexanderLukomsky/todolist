...
type TasksReducerType = RemoveTaskACType | AddTaskACType | AddNewTodolistHandlerACType | ChangeTaskStatusACType | EditTaskTitleACType
export const tasksReducer = (state: TasksStateType, action: TasksReducerType) => {
    switch (action.type) {
        ...
        case 'ADD-EMPTY-TASKLIST-FOR-NEW-TODOLIST':
        ...
        case 'EDIT-TASK-TITLE': return { ...state, [action.payload.todolistID]: state[action.payload.todolistID].map(el => el.id === action.payload.taskID ? { ...el, ...action.payload } : el) }
